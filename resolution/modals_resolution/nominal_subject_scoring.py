...
def nsubj_same_sen_scoring(Verb_list_same_sen, licensor_details, verb_candidate_details_dict, temp_entry_output_storage_file):
    ...
    for each_key in Verb_list_same_sen:
        ...
        elif licensor_details["nominal_subject"] == "you" and verb_candidate_details_dict[each_key]["nominal_subject"] == -1:
            print("%s IMPERATIVE SUBJECT AND MODAL ELLIPSIS SAME SENT + 1" % (each_key), file = temp_entry_output_storage_file)
    ...
    for i in range(0, len(Verb_lists_previous_sens)):
        ...
        if len(Verb_lists_previous_sens[i]) > 0:
            ...
            for each_key in Verb_lists_previous_sens[i]:
                ...
                if verb_noun_subject == -1 and noun_subject in first_person_pronouns:
                    Verb_lists_previous_sens[i][each_key] += 1
                    print("Prev %s - %s IMPERATIVE SUBJECT AND MODAL ELLIPSIS RESPONSE + 1" % (prev_sen_rank, each_key), file = temp_entry_output_storage_file)
