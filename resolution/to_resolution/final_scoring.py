...
from resolution.to_resolution.nominal_subject_scoring import nsubj_same_sen_scoring, nsubj_prev_sen_scoring
from resolution.to_resolution.complement_clause_scoring import score_complement_clause_same_sen, score_complement_clause_prev_sen
from resolution.to_resolution.aux_scoring import aux_scoring_same_sen, aux_scoring_prev_sen
from resolution.to_resolution.verb_list_cleaners import verb_list_cleaner_same_sen, verb_list_cleaner_prev_sen
...
def backtracking_score_same_sen(vpe_sent_dict, Verb_list_same_sentence):
    ...
    # Since ellipsis is for the "To" category the parent index will be this
...
def resolve_to(Parsed_vpe_sen, Verb_list_same_sen, all_sen_vpe, vpe_sent_dict, Parsed_prev_sents, Verb_lists_previous_sens, sentence_speakers_list, sen_indices, parsed_conversation, temp_entry_output_storage_file):
    ...
    for i in range(0, len(Verb_lists_previous_sens)):
        Verb_lists_previous_sens_cleaned.append(verb_list_cleaner_prev_sen(Parsed_prev_sents[i], Verb_lists_previous_sens[i], Parsed_vpe_sen, ellipsis_site))
