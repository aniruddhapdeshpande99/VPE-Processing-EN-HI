...
from resolution.to_do_resolution.nominal_subject_scoring import nsubj_same_sen_scoring, nsubj_prev_sen_scoring
from resolution.to_do_resolution.complement_clause_scoring import score_complement_clause_same_sen, score_complement_clause_prev_sen
from resolution.to_do_resolution.aux_scoring import aux_scoring_same_sen, aux_scoring_prev_sen
from resolution.to_do_resolution.verb_list_cleaners import verb_list_cleaner_same_sen, verb_list_cleaner_prev_sen
...
def backtracking_score_same_sen(vpe_sent_dict, Verb_list_same_sentence):
    ...
    # Step 16.3.4 NEW
    # If the nearest backtracked verb to a To_do licensor is a direct parent to the 'To_Do' licensor with conjunction relation between
    # them, then do not assign extra +1 score to that verb
    if Parsed[row_index][1] == str(backtracking_verb+1) and Parsed[row_index][2] == 'conj':
        backtracking_verb = -1
...
def resolve_to_do(Parsed_vpe_sen, Verb_list_same_sen, all_sen_vpe, vpe_sent_dict, Parsed_prev_sents, Verb_lists_previous_sens, sentence_speakers_list, sen_indices, parsed_conversation, temp_entry_output_storage_file):
